...
IntPaths ZigZag(const IntPath &contour, const IntPaths &holes, double distance, double scale) {
    ...
    struct InterPoint {
        ...
        InterPoint(int x, int y, unsigned int Path, unsigned int point) : point(IntPoint({x, y})), id_path(Path),
                                                                          id_point(point) {}
        ...
        static bool next_to(const InterPoint &a, const InterPoint &b, int size) {
            ...
            return (a.id_path == b.id_path) &&
                   ((a.id_point == (b.id_point + 1) % size) || (b.id_point == (a.id_point + 1) % size));
        }
    };
    ...
    for (int i = 0; i < contour_now.size(); ++i) {
        ...
        for (int j = 0; j < contour_now[i].size(); ++j) {
            ...
            for (int k = std::min(I, II); k < std::max(I, II); ++k) {
                double lambda = 1.0 * (y0 + k * Distance - contour_now[i][j].y) /
                                (contour_now[i][(j + 1) % contour_now[i].size()].y - contour_now[i][j].y);
                intersection.emplace_back(int(contour_now[i][j].x * (1.0 - lambda) +
                                              contour_now[i][(j + 1) % contour_now[i].size()].x * lambda),
                                          y0 + k * Distance, i, id_point++);
            }
        }
